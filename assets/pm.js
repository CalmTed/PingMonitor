...
var Page = function (_winId) {
    ...
    _this.render = function (_state) {
        ...
        var _toFormat = function (_ms) {
            var _h = Math.floor((_ms) / 1000 / 60 / 60);
            var _m = Math.floor((_ms - (_h * 360000)) / 1000 / 60);
            var _s = Math.floor((_ms - (_h * 360000) - (_m * 60000)) / 1000);
        ...
        var formatExactDateTime = function (_timestamp) {
        ...
        var _getStats = function (_b) {
            var history = _b.history;
            var _changesList = [];
            var _statusList = [];
            var _tempStatus;
            var _tempBeginigTime;
            history.reverse().forEach(function (_b) {
            ...
            if (_changesList.length == 0) {
                ...
                _changesList.forEach(function (_chLEl) {
                    if (!_statusList.find(function (_slEl) { return _slEl.status == _chLEl.status; })) {
                        _statusList.push({
                        ...
                        var _index = _statusList.indexOf(_statusList.find(function (_slEl) { return _slEl.status == _chLEl.status; }));
        ...
        var _renderColor = function (theme) {
            document.querySelector('.root').setAttribute('colorMode', theme);
        };
        var _renderLanguage = function (_config) {
        ...
        var _renderSettingsContent = function (_configData) {
        ...
        var _renderNewConfig = function (_newConfigObj) {
        ...
        var _getImagePickerImageDom = function (_b) {
        ...
        var _addEditEvents = function (_b) {
        ...
        var _getCol4Content = function (history, rowId) {
            ...
            var _stats = _getStats({ history: history });
            ...
            var _getStatusElementDom = function (_b) {
                ...
                _statusElement.innerHTML = "<status>".concat(_this.t(status), "</status> <duration>").concat(_toFormat(duration), "</duration>");
            ...
            if (_chagesList.length == 0) {
                ...
                _chagesList.forEach(function (_chLEl, _i) {
                    ...
                    _changeElement.innerHTML = "<status>".concat(_this.t(_chLEl.status), "</status> <duration>").concat(_toFormat(_chLEl.duration), "</duration>");
                    if (_i == 0) {
                    ...
                    var _formatedDateFrom = formatExactDateTime(_chLEl.from);
                    var _formatedDateUntil = formatExactDateTime(_chLEl.until);
        ...
        if (initialRender) {
            ...
            domRoot.innerHTML = "";
            _renderColor(_this.appConfig.colorMode);
            ...
            if (typeof _this.appConfig != 'undefined') { //render settings data if posible
                _renderSettingsContent(_this.appConfig);
            }
                ...
                switch (diffUnit.key) {
                    ...
                    case 'recreateByPosition':
                        ...
                        _recreationRowsObjList.forEach(function (_ro) {
                            _rowListForRecreation_1.append(_createRowDOM(_ro));
                        });
                        break;
                    default:
            ...
            var _renderRowGroup_1 = function (diffUnit) {
                ...
                var _checkGraph = function (_diffUnit) {
                    if (document.querySelector("row[id=\"".concat(_diffUnit.id, "\"] path")) != null) {
                        ...
                        document.querySelector("row[id=\"".concat(_diffUnit.id, "\"] path")).setAttribute('d', _getPath({ dataArray: targetRowObj.history, width: _svgWidth, height: _svgHeight, length: _this.appConfig.miniGraphShowLimitMINS }));
                    }
                };
